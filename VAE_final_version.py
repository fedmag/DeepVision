...
def train(trainloader, start_epochs, epochs, model, device, optimizer, avg_losses):
    ...
    for epoch in range(start_epochs+1, epochs+1):
        ...
        with torch.no_grad():
            ...
            test_image = Image.open('C:\\Users\\Federico\\Desktop\\Computer Science\\Deep Vision\\Denoise for inpainting\\Code FACES\\occulted_output\\20_percent\\100\\00246.png')
            ...
            ### Printing 10 random images from the test group as well as the ground truth to che the progress of the training
            if epoch % 20 == 0:
                test_dir = 'test'
...
## Loading the model
# checkpoint = torch.load('checkpoint-5-1500-changing.pth.tar')
...
### If we want to use some sharpening filters   
...
#     # Enhancing the image
#     t_image = (test_reconst.view(-1,100,100).cuda()).detach().cpu()
#     t_image = transforms.ToPILImage()(t_image)
#     enhancer = ImageEnhance.Sharpness(t_image)
#     enhanced_test_img = enhancer.enhance(1.5)
#     enhanced_test_img.save('reconst-{}-enhanced.png'.format(epochs), 'PNG')
